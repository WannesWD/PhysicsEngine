// code inspired by https://www.3dgep.com/learning-directx-12-1/#CMake
...
// window callback function foward declaration
...
void ParseCommandLineArguments()
{
	...
	{
        if (std::wcscmp(argv[i], L"-w") == 0 || std::wcscmp(argv[i], L"--width") == 0)
            g_ClientWidth = std::wcstol(argv[++i], nullptr, 10);

        if (std::wcscmp(argv[i], L"-h") == 0 || std::wcscmp(argv[i], L"--height") == 0)
            g_ClientHeight = std::wcstol(argv[++i], nullptr, 10);
        
        if (std::wcscmp(argv[i], L"-warp") == 0 || std::wcscmp(argv[i], L"--warp") == 0)
            g_UseWarp = true;
	}
...
HWND CreateDXWindow(const wchar_t* windowClassName, HINSTANCE hInst, const wchar_t* windowTitle,
    uint32_t width, uint32_t height)
{
    ...
    int windowX = std::max<int>(0, (screenWidth - windowWidth) * .5);
    int windowY = std::max<int>(0, (screenHeight - windowHeight) * .5);
...
int CALLBACK wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR lpCmdLine, int nCmdShow)
{
    ...
    g_hWnd = CreateDXWindow(windowClassName, hInstance, L"Learning DirectX 12",
